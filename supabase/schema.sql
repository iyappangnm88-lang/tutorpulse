...
-- ==============================================================================
-- NUZILO MASTER PRODUCT TRANSFORMATION EXTENSIONS
-- ==============================================================================

-- 1. Student Profiles Gamification Fields
...
-- 7. Server-Authoritative RPC: submit_fast_answer
...
BEGIN
    ...
    SELECT * INTO v_question FROM public.classroom_questions WHERE id = p_question_id;
    ...
    SELECT * INTO v_session FROM public.class_sessions WHERE id = v_question.class_session_id;

    SELECT id INTO v_existing_answer_id FROM public.classroom_answers WHERE question_id = p_question_id AND user_id = v_user_id;
    ...
    WHERE stc.student_user_id = v_user_id AND stc.tutor_id = v_question.tutor_id AND stc.status = 'active'
    LIMIT 1;

    IF v_is_correct THEN
        SELECT COUNT(*) + 1 INTO v_rank FROM public.classroom_answers WHERE question_id = p_question_id AND is_correct = true;
        v_base_xp := v_question.points_xp;
        v_base_coins := v_question.coins_reward;

        IF v_rank = 1 AND v_question.first_x_count >= 1 THEN v_bonus_coins := 5;
        ELSIF v_rank = 2 AND v_question.first_x_count >= 2 THEN v_bonus_coins := 3;
        ELSIF v_rank = 3 AND v_question.first_x_count >= 3 THEN v_bonus_coins := 2;
        ELSIF v_rank <= v_question.first_x_count THEN v_bonus_coins := 1;
        END IF;
    ...
    INSERT INTO public.classroom_answers (
        question_id, class_session_id, user_id, student_id,
        selected_option, answer_text, is_correct, answer_time_ms,
        awarded_xp, awarded_coins, rank_position, submitted_at
    )
    VALUES (
        p_question_id, v_question.class_session_id, v_user_id, v_student_id,
        p_selected_option, p_answer_text, v_is_correct, v_elapsed_ms,
        v_base_xp, v_total_coins, CASE WHEN v_is_correct THEN v_rank ELSE NULL END, v_now
    )
    RETURNING id INTO v_new_answer_id;

    IF v_is_correct AND v_total_coins > 0 THEN
        INSERT INTO public.gold_coin_transactions (
            student_user_id, amount, transaction_type, source,
            classroom_session_id, activity_id, description, metadata
        )
        VALUES (
            v_user_id, v_total_coins,
            CASE WHEN v_bonus_coins > 0 THEN 'FIRST_X_BONUS' ELSE 'FAST_ANSWER' END,
            'classroom', v_question.class_session_id, v_question.id,
            format('Fast answer reward for question #%s (Rank %s)', v_question.order_index + 1, COALESCE(v_rank, 0)),
            jsonb_build_object('question_id', v_question.id, 'rank', v_rank, 'bonus_coins', v_bonus_coins, 'elapsed_ms', v_elapsed_ms)
        );

        UPDATE public.student_profiles
        SET xp = xp + v_base_xp, gold_coins_balance = gold_coins_balance + v_total_coins, updated_at = NOW()
        WHERE id = v_user_id;

        IF v_rank = 1 THEN
            INSERT INTO public.student_badges (student_user_id, badge_id, classroom_session_id)
            SELECT v_user_id, b.id, v_question.class_session_id FROM public.badges b WHERE b.slug = 'speed_champion'
            ON CONFLICT DO NOTHING;
        END IF;

        IF v_elapsed_ms <= 5000 THEN
            INSERT INTO public.student_badges (student_user_id, badge_id, classroom_session_id)
            SELECT v_user_id, b.id, v_question.class_session_id FROM public.badges b WHERE b.slug = 'quick_learner'
            ON CONFLICT DO NOTHING;
        END IF;
    END IF;
...
-- 8. Server-Authoritative RPC: award_attendance_reward
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        WHERE student_user_id = p_student_user_id AND classroom_session_id = p_session_id AND transaction_type = 'ATTENDANCE_REWARD'
    ...
    INSERT INTO public.gold_coin_transactions (
        student_user_id, amount, transaction_type, source, classroom_session_id, description, metadata
    )
    VALUES (
        p_student_user_id, p_reward_coins, 'ATTENDANCE_REWARD', 'attendance',
        p_session_id, format('Full class attendance reward for session: %s', v_session.title),
        jsonb_build_object('session_id', p_session_id, 'xp', p_reward_xp)
    );

    UPDATE public.student_profiles
    SET xp = xp + p_reward_xp, gold_coins_balance = gold_coins_balance + p_reward_coins, updated_at = NOW()
    WHERE id = p_student_user_id;

    INSERT INTO public.student_badges (student_user_id, badge_id, classroom_session_id)
    SELECT p_student_user_id, b.id, p_session_id FROM public.badges b WHERE b.slug = 'first_class'
    ON CONFLICT DO NOTHING;

    RETURN jsonb_build_object('success', true, 'coins_awarded', p_reward_coins, 'xp_awarded', p_reward_xp);
END;
$$;
